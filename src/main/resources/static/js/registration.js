...
(function() {
	...
	const pond=createFilepond('#form-registration-avatar-upload-filepond');
	const pondRoot=$('#register-form .filepond--root');
	const defaultAvatar=$('#register-form #filepond--default-avatar');
	const pondFileInput=$('#form_registration_avatar_upload');
	const formInputs=$('#register-form .form-group input');
	...
	addDefaultAvatar("#register-form .filepond--drop-label")
	handleFormInputChange(formInputs, $('#register-form .btn-send'));
	submitRegistrationForm($('#register-form'), formInputs);
	handleRegistrationSucces($("meta[name='state']"), $('#modal-activator-button'));
	handleRegistrationModalClose($('#registrationSuccesModal'));
...
function submitRegistrationForm(form, formInputs){
	form.submit(event=>{
		event.preventDefault();
...
function handleRegistrationSucces(stateMeta, modalActivatorButton){
	if (stateMeta.attr("content")=="success")
		modalActivatorButton.click();
}

function handleRegistrationModalClose(registrationModal){
	registrationModal.find("button.btn-secondary, button.close").click((event)=>{
		event.preventDefault();

		registrationModal.modal('hide')
		registrationModal.one('hidden.bs.modal', ()=>{
			htmlBody.trigger("refreshContentToHome");
